/* =====================================================================
 * Report Designer - config.js
 * Free-layout report designer for Kintone.
 * Units: all element geometry is stored in mm. Pixels only at render.
 * VERSION below is the single source of truth for the whole code bundle
 * (common.js / config.js / desktop.js / config.css). Bump it on every change.
 * ===================================================================== */
(function (PLUGIN_ID) {
  'use strict';
  ...
  var I18N = {
    ja: {
      ...
      pageShow: '表示ページ', psFirst: '1ページ目のみ', psAll: '全ページ', psLast: '最終ページのみ',
      footerWidow: '最終ページ最小行数',
      previewPage: 'ページ',
    ...
    en: {
      ...
      cellAutoFit: 'Shrink cell text to fit',
      ...
      pageShow: 'Show on', psFirst: 'First page only', psAll: 'All pages', psLast: 'Last page only',
      footerWidow: 'Min rows on last page',
      previewPage: 'Page',
  ...
  function renderProps() {
    ...
    if (el.type === 'text' || el.type === 'field') {
      ...
      h2 += propRow(t('cellAutoFit'), chk(el, 'cellAutoFit'));
      ...
      if (el.paginate) {
        ...
        h2 += propRow(t('footerWidow'), inp(el, 'footerWidow', 'number', 'min="0" max="20" step="1" placeholder="2"'));
      }
    ...
    if (el.type !== 'table' || !el.paginate) {
      var psCur = RPTC.pageShowOf(el); // migrates the old repeatAllPages boolean
      h2 += propRow(t('pageShow'), (function () {
        var s = '<select data-prop="pageShow"' +
          (anyPaginate ? '' : ' disabled title="改ページするテーブルがある場合のみ有効"') + '>';
        [{ v: 'first', l: t('psFirst') }, { v: 'all', l: t('psAll') }, { v: 'last', l: t('psLast') }]
          .forEach(function (o) {
            s += '<option value="' + o.v + '"' + (psCur === o.v ? ' selected' : '') + '>' + esc(o.l) + '</option>';
          });
        return s + '</select>';
      })());
    }
  ...
  function renderFooterEditor(el) {
    ...
    (el.footers || []).forEach(function (f, i) {
      ...
      html += propRow(t('conds'), '<button type="button" class="rpt-btn-mini" data-fcond="' + i + '" style="flex:1;text-align:left;">' +
        esc(condBtnLabel(f)) + '</button>');
    ...
    wrap.querySelectorAll('[data-fcond]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var i = Number(btn.getAttribute('data-fcond'));
        // condition on the same set of fields a footer can bind to, so
        // "hide 特別値引 when 特別値引 = 0" needs no extra plumbing
        var opts;
        if (el.subtableCode === '$RECORDS') {
          opts = [{ v: '', l: t('fieldNotSet') }, { v: 'RCOUNT', l: t('aggRcount') }];
          state.fields.forEach(function (fd) { opts.push({ v: 'TSUM.' + fd.code, l: 'Σ ' + fd.label }); });
          opts = opts.concat(fieldSelOptions().slice(1));
        } else {
          opts = fieldSelOptions();
        }
        openCondEditor(el.footers[i], opts);
      });
    });
  ...
  function helpBasicHtml() {
    if (LANG === 'ja') {
      ...
      '<h3>条件書式</h3>' +
      '<p>要素・テーブル列・<b>フッター行</b>の「⚡条件書式」で、値に応じて文字色/背景/太字/非表示を切り替えます。' +
      '列の条件は行ごとの値で判定されます(例: 数量=0 を赤)。' +
      'フッター行の「非表示」は<b>行ごと消えます</b>(例: 特別値引=0 のとき行を出さない)。改ページの高さ計算にも反映されます。</p>' +
      '<h3>複数ページ(改ページ)</h3>' +
      '<p>テーブルの「改ページする」をONにすると、収まらない行が2ページ目以降に流れます。' +
      '各要素の<b>表示ページ</b>で「1ページ目のみ / 全ページ / 最終ページのみ」を選べます' +
      '(ロゴ・見積番号は「全ページ」、備考は「最終ページのみ」)。' +
      '<code>{PAGE}</code> <code>{PAGES}</code>でページ番号を印字できます。' +
      'フッター行は最終ページのみに出ます。<b>最終ページ最小行数</b>で、合計行だけが最終ページに残るのを防ぎます。</p>' +
      '<h3>ラベル出力(行毎)</h3>' +
    ...
    return '' +
      ...
      '<h3>Multi-page</h3><p>Enable "Paginate" on the table. Each element has a <b>Show on</b> setting: ' +
      'first page only / all pages / last page only. Use <code>{PAGE}</code>/<code>{PAGES}</code> for page numbers. ' +
      'Footers print on the last page; <b>Min rows on last page</b> stops the totals stranding there alone.</p>' +
    ...
    (isRecMode
      ...
      return RPTC.renderPages(tpl, rec || {}, state.images).then(function (allPages) {
        ...
        pages.forEach(function (page, i) {
          ...
          RPTC.autofitText(pageDiv); // preview must shrink cells the same way print does
        });
      });
    }).catch(function (e) {
      console.error(e);
  ...
  function buildConf() {
    ...
    /* manifest.json declares required_params:["templates"], so that key MUST be
     * present or kintone marks the plugin unconfigured and desktop.js never
     * runs. Keep the original single-key format whenever it fits; only chunk
     * when it cannot, and then leave a valid placeholder behind so kintone's
     * required-param check still passes. */
    var tplJson = JSON.stringify(state.templates);
    if (tplJson.length <= RPTC.VALUE_CHUNK) {
      conf.templates = tplJson;
    } else {
      conf.templates = '[]';
      var tplKeys = RPTC.chunkValue('tpl_', tplJson);
  ...
  function applyI18nLabels() {
    var lg = document.getElementById('rpt-version');
    if (lg) lg.textContent = 'v' + VERSION;
    var map = {
  ...
  function init() {
    ...
    if (root) root.innerHTML = RPT_HTML;
    cacheEls();
    ensureColorPalette();
    applyI18nLabels();
    loadConfig();
